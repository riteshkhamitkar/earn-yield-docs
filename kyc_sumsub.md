...
│  │  ─────────────────────────────────────────                │ │
│  │  Collects: Name, DOB, Address, Phone, Tax ID,            │ │
│  │  Occupation, Source of Funds, Expected Monthly Volume     │ │
│  │                                                           │ │
│  │  Step 2: ☐ ID Verification (Sumsub native SDK)           │ │
│  │  ─────────────────────────────────────────                │ │
│  │  Scans: Government ID (passport/DL/national ID)          │ │
│  │  + Selfie/Liveness check                                  │ │
│  │                                                           │ │
│  │  Step 3: ☐ Terms & Conditions (Bridge TOS WebView)       │ │
│  │  ─────────────────────────────────────────                │ │
│  │  Shows Bridge's TOS, captures signed_agreement_id         │ │
│  │                                                           │ │
│  │  Step 4: ☐ Proof of Address (if required)                │ │
│  │  ─────────────────────────────────────────                │ │
...
|---|---|---|
| Occupation | Our questionnaire → `KycProfile.occupation` | `most_recent_occupation` |
| Employment status | Our questionnaire → `KycProfile.employmentStatus` | `employment_status` |
| Source of funds | Our questionnaire → `KycProfile.sourceOfFunds` | `source_of_funds` |
| Account purpose | Our questionnaire → `KycProfile.accountPurpose` | `account_purpose` |
| Expected monthly volume | Our questionnaire → `KycProfile.expectedMonthlyVolume` | `expected_monthly_payments` |
| Acting as intermediary | Our questionnaire → `KycProfile.actingAsIntermediary` | `acting_as_intermediary` |
...
1. Our backend calls Bridge's TOS endpoint to get a TOS URL
2. Mobile app shows the TOS in a WebView
3. When user accepts, the WebView fires a `postMessage` with the `signed_agreement_id`
4. We store this and include it in the customer creation payload
...
│  │        issuing_country: "IND" }                                │   │
│  │    ],                                                          │   │
│  │    most_recent_occupation: "software_engineer", ← Questionnaire│   │
│  │    source_of_funds: "salary",                 ← Questionnaire  │   │
│  │    expected_monthly_payments: "1000_4999"      ← Questionnaire │   │
│  │  }                                                             │   │
│  └──────────────────────────────────────────────────────────────┘  │
│                              │                                     │
│                              ▼                                     │
...
POST /api/v1/kyc/questionnaire
{
  ...
  "accountPurpose": "receive_freelance_payments",
...
Returns: `{ "tosLink": "https://dashboard.bridge.xyz/accept-terms-of-service?..." }`

App shows WebView. Rohan reads and accepts. WebView fires `postMessage` with `signed_agreement_id: "sa_xyz789"`.
...
```json
{
  ...
  "most_recent_occupation": "software_engineer",
  "employment_status": "employed",
  "source_of_funds": "salary",
  "account_purpose": "receive_freelance_payments",
  "expected_monthly_payments": "1000_4999",
...
model KycProfile {
  ...
  occupation             String?  // Dropdown value key
  employmentStatus       String?  // employed, self_employed, unemployed, retired, student
  annualSalary           String?  // Range key
  sourceOfFunds          String?  // salary, business_income, investments, inheritance, etc.
  accountPurpose         String?  // Dropdown value key
  expectedMonthlyVolume  String?  // Range key (e.g., "1000_4999")
  actingAsIntermediary   String?  // "yes" or "no"
...
| `GET` | `/kyc/tos-link/:partnerId` | Get TOS URL for a partner |
| `POST` | `/kyc/tos` | Save TOS acceptance |
